...
RECORD_ID="709dc57daa1f65ae40962bfa3e4cee43"  # 你要更新的 DNS 记录的 ID
AUTH_KEY="Ip9gxc9QqfPx-pgvn8uHXi_KjVGEUeYjXJITKhrM"  # 你的 API Token
DOMAIN="awshk3.tfvou.com"  # 你想更新的域名或子域名
...
if [ "$CURRENT_IP" == "$DNS_IP" ]; then
  MESSAGE="DDNS检测：AWSHK三 IP 无变化，当前 IP 为 $CURRENT_IP。无需更新。"
  echo $MESSAGE
  TG_BOT "$MESSAGE"
else
  TTL=60  # 设置TTL为60秒
  ...
  if [[ $RESPONSE == *"\"success\":true"* ]]; then
      MESSAGE="AWSHK三 DNS 记录已成功更新为 $CURRENT_IP，TTL 为 $TTL 秒。"
      echo $MESSAGE
      TG_BOT "$MESSAGE"
  else
      MESSAGE="AWSHK三 DNS 记录更新失败。响应: $RESPONSE"
      echo $MESSAGE
      TG_BOT "$MESSAGE"
  fi
fi
