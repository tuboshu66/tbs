...
RECORD_ID="d5556b6c7a2d8f112bb557de9ebb6989"  # 你要更新的 DNS 记录的 ID
AUTH_KEY="Ip9gxc9QqfPx-pgvn8uHXi_KjVGEUeYjXJITKhrM"  # 你的 API Token
DOMAIN="awssg1.tfvou.com"  # 你想更新的域名或子域名
...
if [ "$CURRENT_IP" == "$DNS_IP" ]; then
  MESSAGE="AWSSG一 IP 无变化，当前 IP 为 $CURRENT_IP。无需更新。"
  echo $MESSAGE
  TG_BOT "$MESSAGE"
else
  TTL=60  # 设置TTL为60秒
  ...
  if [[ $RESPONSE == *"\"success\":true"* ]]; then
      MESSAGE="AWSSG一 DNS 记录已成功更新为 $CURRENT_IP，TTL 为 $TTL 秒。"
      echo $MESSAGE
      TG_BOT "$MESSAGE"
  else
      MESSAGE="AWSSG一 DNS 记录更新失败。响应: $RESPONSE"
      echo $MESSAGE
      TG_BOT "$MESSAGE"
  fi
fi
