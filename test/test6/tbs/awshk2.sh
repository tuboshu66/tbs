...
log "下载并运行 DDNS 脚本..."
if wget -N -O /root/scripts/DDNSawshk2.sh https://raw.githubusercontent.com/tuboshu66/tbs/master/test/test6/tbs/DDNSawshk2.sh && chmod +x /root/scripts/DDNSawshk2.sh; then
    /root/scripts/DDNSawshk2.sh
    log "成功执行 DDNS 脚本。"
    ...
    # 添加到 crontab
    if (crontab -l; echo "*/5 * * * * /root/scripts/DDNSawshk2.sh >> /var/log/ddns.log 2>&1") | crontab -; then
