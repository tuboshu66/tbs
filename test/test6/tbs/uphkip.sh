...
if [ -f /root/tbs/test/test6/tbs/hkip1.txt ]; then
  ...
  TG_MESSAGE="hkip1.txt 文件不存在，请检查路径。"
  TG_BOT
  echo "$TG_MESSAGE"
  exit 1
fi
...
# 确保原有 IP 地址不是空值
if [ -z "$old_ip" ]; then
  TG_MESSAGE="原有 IP 地址为空，请检查 hkip1.txt 文件内容。"
  TG_BOT
  echo "$TG_MESSAGE"
  exit 1
fi
...
if [ "$new_ip" != "$old_ip" ]; then
  ...
  if sed -i "s/$old_ip/$new_ip/g" /root/tbs/test/test6/tbs/tbs.conf; then
    ...
    TG_MESSAGE="更新 tbs.conf 文件中的 IP 失败。"
    TG_BOT
    echo "$TG_MESSAGE"
    exit 1
  fi
  ...
  if echo "$new_ip" > /root/tbs/test/test6/tbs/hkip1.txt; then
    TG_MESSAGE="IP 更新成功: $old_ip -> $new_ip"
    TG_BOT
    echo "$TG_MESSAGE"
  else
    TG_MESSAGE="更新 hkip1.txt 文件中的 IP 失败。"
    TG_BOT
    echo "$TG_MESSAGE"
    exit 1
  fi
else
  TG_MESSAGE="AWSHK1 IP 无变化"
  TG_BOT
  echo "$TG_MESSAGE"
fi
