...
if curl -L https://raw.githubusercontent.com/nezhahq/scripts/main/install.sh -o /root/scripts/nezha.sh && chmod +x /root/scripts/nezha.sh; then
    # 第一次运行 Nezha 安装脚本
    /root/scripts/nezha.sh install_agent vps.tbstbs168.com 5555 06bfd2cd0abcc357d6
...
log "下载并运行 DDNS 脚本..."
if wget -N -O /root/scripts/DDNSlinode2.sh https://raw.githubusercontent.com/tuboshu66/tbs/master/test/test6/tbs/DDNSlinode2.sh && chmod +x /root/scripts/DDNSlinode2.sh; then
    /root/scripts/DDNSlinode2.sh
    log "成功执行 DDNS 脚本。"
    ...
    # 添加到 crontab
    if (crontab -l; echo "*/5 * * * * /root/scripts/DDNSlinode2.sh >> /var/log/ddns.log 2>&1") | crontab -; then
